...
CREATE DEFINER=`student`@`%` PROCEDURE `new_procedure`()
BEGIN
   DECLARE counter INT DEFAULT 1;
    

    WHILE counter <= 100000 DO
        INSERT INTO Nums values(counter);
...
SELECT E.empid, E.firstname, E.lastname, fiveNums.n
FROM Employees AS E
  CROSS JOIN 
    (select  n 
   from Nums 
   order by n limit 5) AS fiveNums
order by n, empid
...
-- Solution
SELECT E.empid, 
  DATE_ADD("2009-06-12", interval D.n -1 DAY) as dt
  from Employees as E
  cross join Nums as D
  WHERE D.n <= DATEDIFF( "2009-06-16", "2009-06-12") +1
  order by empid, dt;
...
SELECT DISTINCT C.custid, C.companyname, 
  CASE WHEN O.orderid IS NOT NULL THEN 'Yes' ELSE 'No' END AS "Has Orders On 2007 02 12"
FROM Customers AS C
  LEFT JOIN Orders AS O
    ON O.custid = C.custid
